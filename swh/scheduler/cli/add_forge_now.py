...
@add_forge_now.command("check-listed-origins")
@click.argument("lister_name", nargs=1, required=True)
@click.argument("instance_name", nargs=1, required=True)
@click.pass_context
def check_listed_origins_cli(ctx, lister_name, instance_name):

    """
    Check the listed origins in the scheduler.
    """
    ...
    if listed_origins_lister is None:
        exit(f"{instance_name} with {lister_name} lister is {listed_origins_lister}.")
    else:
        listed_origins_id = listed_origins_lister.id

    listed_origins = scheduler.get_listed_origins(
        lister_id=listed_origins_id,
        enabled=None,
        limit=100000,
    ).results
    ...
    if len(listed_origins) == 0:
        exit(
            f"{instance_name} with {lister_name} lister has {len(listed_origins)} origin."
        )

    print(f"{instance_name} has {len(listed_origins)} origins.")
    for origin in listed_origins:
        print(origin.last_seen, origin.url)
